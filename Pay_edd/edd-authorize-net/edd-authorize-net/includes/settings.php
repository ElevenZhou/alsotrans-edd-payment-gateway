	...
	$edda_settings = array(
		'authorize' => array(
			...
			array(
				...
				'desc' => __( 'Enter your authorize.net signature key. To generate a Signature Key, follow this guide:', 'edda' ) . ' <a href="https://easydigitaldownloads.com/docs/authorize-net-payment-gateway/" target="_blank">' . __( 'How to create signature key for Authorize.net', 'edda' ) . '</a>',
				'type' => 'password',
			),
		),
	);
...
/**
 * Update the docs link for Authorize.net settings.
 *
 * @since 2.1.0
 * @param string $link The current link
 * @return string The updated link
 */
function edd_authorize_net_docs_link( $link ) {
	$page    = filter_input( INPUT_GET, 'page', FILTER_SANITIZE_SPECIAL_CHARS );
	$section = filter_input( INPUT_GET, 'section', FILTER_SANITIZE_SPECIAL_CHARS );

	if ( 'edd-settings' === $page && 'authorize' === $section ) {
		$link = 'https://easydigitaldownloads.com/docs/authorize-net-payment-gateway/';
	}

	return $link;
}
add_filter( 'edd_flyout_docs_link', 'edd_authorize_net_docs_link' );
