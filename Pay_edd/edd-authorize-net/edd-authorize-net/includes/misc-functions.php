...
function edda_process_payment( $purchase_data ) {
	...
	$payment_id = edd_build_order( $payment_data );
	if ( ! $payment_id ) {
	...
	try {
		...
		edd_record_gateway_error( $e->getPaymentErrorCode(), $e->getMessage(), $payment_id );
		edd_update_order_status( $payment_id, 'failed' );
		edd_redirect( edd_get_failed_transaction_uri( '?payment-id=' . sanitize_key( $payment_id ) ) );
	}
}
